...
import { invitarStaff } from './actions';

export function InvitarStaffForm() {
  const [abierto, setAbierto] = useState(false);
  const router = useRouter();
  const [error, formAction, pending] = useActionState(async (_prev: string | undefined, fd: FormData) => {
    const res = await invitarStaff(_prev, fd);
    if (res === undefined) {
      setAbierto(false);
      router.refresh();
    }
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-jab-muted">
            Le llega un mail para elegir su contraseña. Después le das acceso a los clientes que
            le correspondan desde acá.
          </p>
        </div>
